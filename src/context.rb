...
module ZCA
  ...
  module Context
    ...
    def self.login_with_cookie(cookie_path:, imei:, user_agent: nil)
      json = File.read(cookie_path)
      cookie_jar = ZCA::Cookie::CookieJar.from_json(json)
      context = create_context(imei: imei)
      context.user_agent = user_agent if user_agent
      context.cookie = cookie_jar
      api_login = ZCA::API::Login.new(context)
      login_result = api_login.login(encrypt_params: false)
      if login_result.is_a?(Hash)
        zfamily = login_result['zfamily'] || (login_result['data'] && login_result['data']['zfamily'])
        context.secret_key = zfamily['enk'] if zfamily && zfamily['enk']
        context.viewerkey  = zfamily['viewer_key'] if zfamily && zfamily['viewer_key']
